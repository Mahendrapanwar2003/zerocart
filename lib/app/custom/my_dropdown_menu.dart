...
class _MyDropdownMenuState<T> extends State<MyDropdownMenu<T>> {
  ...
  void initState() {
    ...
    final int index = filteredEntries.indexWhere((MyDropdownMenuEntry<T> entry) => entry.value == widget.initialSelection);
    if (index != -1) {
...
class _MyDropdownMenuBody extends MultiChildRenderObjectWidget {
  const _MyDropdownMenuBody({
    super.children,
    this.width,
  });
  ...
  @override
  _RenderMyDropdownMenuBody createRenderObject(BuildContext context) {
    return _RenderMyDropdownMenuBody(
      width: width,
    );
  }
}

class _MyDropdownMenuBodyParentData extends ContainerBoxParentData<RenderBox> { }

class _RenderMyDropdownMenuBody extends RenderBox
    with ContainerRenderObjectMixin<RenderBox, _MyDropdownMenuBodyParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, _MyDropdownMenuBodyParentData> {

  _RenderMyDropdownMenuBody({
    this.width,
  });
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! _MyDropdownMenuBodyParentData) {
      child.parentData = _MyDropdownMenuBodyParentData();
    }
  }
  ...
  void performLayout() {
    ...
    while (child != null) {
      if (child == firstChild) {
        ...
        final _MyDropdownMenuBodyParentData childParentData = child.parentData! as _MyDropdownMenuBodyParentData;
